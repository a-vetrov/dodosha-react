...
const Breadcrumb: React.FC<{items: Array<BreadcrumbItemType>}> = ({items}) => {
    ...
    return (
        <div className={styles.container}>
            ...
            <BreadcrumbDelimiter />
            {
                items.map((item, index) => <BreadcrumbItem  item={item} showDelimiter={index < items.length - 1} key={item.caption}/>)
            }
        </div>
    )
}
